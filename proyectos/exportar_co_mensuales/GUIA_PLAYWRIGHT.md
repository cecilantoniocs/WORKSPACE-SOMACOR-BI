...
todos los reportes del workspace **"C.O - Mensuales"** como archivos PDF, igual
...
Al principio se intento usar la **API oficial de PowerBI**. No funciono porque esa API
exige que el workspace este en **capacidad Premium o Fabric dedicada**. Los workspaces de
C.O estan en **capacidad compartida (Pro)**, asi que la API rechaza la exportacion con el
error *"Report requested for export is not on dedicated capacity"*.

Por eso se usa este metodo con **Playwright**, que controla un navegador real y hace los
mismos clics que haria una persona (Exportar > PDF > Exportar). Como usa la interfaz
normal de PowerBI, **no tiene restriccion de licencia** y funciona con la cuenta Pro.
...
- Entra al workspace "C.O - Mensuales" en PowerBI.
...
- Asi arma la lista completa de los reportes sin perder ninguno. Mientras lo hace,
...
```
Control Operacional 360 Mensual - ANGLO WEST WALL.pdf
```
...
Quedan todos en:
```
c:\WORKSPACE SOMACOR\proyectos\exportar_co_mensuales\salidas\
```
...
```
c:\WORKSPACE SOMACOR\proyectos\exportar_co_mensuales\datos\sesion_browser\
```
...
(unos 12-15 segundos por reporte). Es normal que tarde varios minutos.
...
| `WORKSPACE_ID` | ID del workspace de PowerBI a exportar | 8d1feccb-...0e24 |
